...
func TestSweepOrphanPIDPrefixedDirsKillsLiveTmuxServerBeforeRemoval(t *testing.T) {
	...
	// so this needs a SHORT root -- the same path-length reason production's
	// own NewSocketParentDir("/tmp", ...) call sites have. testutil.ShortTempDir
	// picks that root per platform ("/tmp" on darwin, %LOCALAPPDATA%\gct on
	// Windows, where a bare "/tmp" is not absolute and resolves against the
	// current drive) and registers its own cleanup.
	root := testutil.ShortTempDir(t, "tmuxtest-orphan-")
