...
func mustOpenSiblingLock(t *testing.T, path string) *os.File {
	t.Helper()
	// O_RDWR rather than O_WRONLY|O_APPEND: Windows append-only handles lack
	// the access rights LockFileEx requires (Access is denied). Dropping
	// O_APPEND also drops append POSITIONING, so callers that write through
	// this handle must be positioned at the end explicitly — seek here so the
	// handle behaves like the append-mode one it replaces on both platforms.
	...
	if _, err := f.Seek(0, io.SeekEnd); err != nil {
		_ = f.Close()
		t.Fatalf("seek sibling to end: %v", err)
	}
