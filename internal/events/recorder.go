...
func (r *FileRecorder) Record(e Event) {
	...
	if err := lockRecorderFile(r.file, r.path); err != nil {
...
// lockRecorderFile takes the exclusive cross-process lock on f, retrying while
// another writer holds it until recordFlockTimeout elapses.
//
// It takes *os.File rather than a raw fd and goes through
// tryLockRecorderFile/recorderLockWouldBlock so the retry policy is shared by
// both platforms: Windows has no flock, and its LockFileEx-based
// implementation lives in recorder_flock_windows.go.
func lockRecorderFile(f *os.File, path string) error {
	deadline := time.Now().Add(recordFlockTimeout)
	for {
		err := tryLockRecorderFile(f)
		if err == nil {
			return nil
		}
