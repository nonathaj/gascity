...
func buildOnDeath(a *Agent, includeEphemeralInProgress bool) string {
	...
	return `{ ` +
		...
		ephemeralRead +
		`} | ` +
		// tr -d '\r': native Windows jq emits CRLF, so without it the trailing
		// \r lands in routed_to and an empty routed_to tests as non-empty —
		// the gc.run_target backfill branch below could never fire on Windows.
		// On unix jq output has no \r, so this is a no-op.
		`tr -d '\r' | ` +
