...
func runShellWithFakeBd(t *testing.T, shellCmd string, env map[string]string, bdScript string) string {
	...
	commandEnv := []string{"PATH=" + tmp + string(os.PathListSeparator) + os.Getenv("PATH")}
	for k, v := range env {
...
func runShellCommandCapture(t *testing.T, command string, env []string) (stdout, stderr string, exit int) {
	t.Helper()
	// execshim resolves sh on Windows (Git-for-Windows) and testShellEnv carries
	// the system variables and the shell's own directory through the PATH
	// override; on Unix both are identity.
