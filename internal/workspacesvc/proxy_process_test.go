...
func TestMain(m *testing.M) {
	// Hard lifetime bound so a killed `go test` run cannot orphan this binary
	// on Windows (incident gw-qhs). StartExitWatchdog rather than
	// RunWithWatchdog because this TestMain owns its own run/exit structure:
	// the leak sweep below has to happen after m.Run and before the exit.
