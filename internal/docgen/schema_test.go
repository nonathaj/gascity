...
func TestGenerateFormulaSchema(t *testing.T) {
	...
	// Formula properties are in $defs.Formula.
	props := defProperties(t, raw, "Formula")
	for _, expected := range []string{"formula", "description", "steps"} {
		if _, ok := props[expected]; !ok {
			t.Errorf("missing property %q", expected)
		}
	}

	// Check steps has minItems.
	steps, ok := props["steps"].(map[string]interface{})
	if !ok {
		t.Fatal("steps not a map")
	}
	if minItems, ok := steps["minItems"].(float64); !ok || minItems != 1 {
		t.Errorf("steps minItems: got %v, want 1", steps["minItems"])
	}

	// Check required fields.
	defs := raw["$defs"].(map[string]interface{})
	formulaDef := defs["Formula"].(map[string]interface{})
	required, ok := formulaDef["required"].([]interface{})
	if !ok {
	...
	if !reqSet["formula"] {
		t.Error("'formula' not in required list")
	}
}
