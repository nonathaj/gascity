...
//
// The mechanism is now startupDialogBudget: one deadline threaded through every
// acceptor, replacing the per-class floor this test originally poked at.
func TestAcceptStartupDialogsSharesBudgetAcrossClasses(t *testing.T) {
	...
	t.Cleanup(func() { dialogPollInterval = oldInterval })
	...
	// The budget is shared, not skipped: the phase must still actually look at
	// the pane. Asserting only the elapsed ceiling would also pass if the
	// acceptors were removed outright.
	if got := peeks.Load(); got == 0 {
		t.Fatal("peek was never called; the dialog phase did no work at all")
	}
}
