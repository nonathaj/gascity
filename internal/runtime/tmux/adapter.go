...
func (o *tmuxStartOps) runSetupCommand(ctx context.Context, cmd string, env map[string]string, timeout time.Duration) error {
	...
	// execshim.ShPath() rather than a bare "sh": Windows has no sh on PATH by default,
	// so the interpreter is resolved explicitly (doctrine P1). Identity on Unix.
	c := exec.CommandContext(runCtx, execshim.ShPath(), "-c", cmd)
