...
//
// It must match wherever the herdr binary itself resolves its config/state
// directory — confirmed empirically (`XDG_CONFIG_HOME=X herdr --help` prints
// "Config: X/herdr/config.toml" regardless of $HOME; with XDG_CONFIG_HOME unset
// it falls back to "$HOME/.config/herdr/...") to be standard XDG Base Directory
// precedence, i.e. exactly os.UserConfigDir() on this platform. A plain
// os.UserHomeDir()+".config" join (the prior implementation) silently ignores
// XDG_CONFIG_HOME, so it diverges from herdr's own resolution in any
// environment that sets XDG_CONFIG_HOME while pointing $HOME elsewhere — this
// fleet's agent sandboxes do exactly that. The result: this client dials a
// socket no herdr process ever binds, so serverAlive() reads false against a
// perfectly healthy server ("did not become ready"), and every retry launches a
// redundant herdr server contending for the same pane ("agent_pane_busy") —
// ga-nqlb8q.
//
// os.UserConfigDir is also what makes this correct off Unix: it resolves
// %AppData% on Windows, where herdr writes
// %AppData%\herdr\sessions\<session>\herdr.sock. Hardcoding "~/.config" made
// this Unix-only, so gc probed a directory that never exists there and every
// Start failed after burning the full 10s readiness wait.
//
