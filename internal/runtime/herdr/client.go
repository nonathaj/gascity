...
// Must match wherever the herdr binary itself resolves its config/state
...
// Directory precedence, i.e. exactly os.UserConfigDir() on each platform:
// $XDG_CONFIG_HOME or ~/.config on Unix, and %AppData% on Windows.
//
// A plain os.UserHomeDir()+".config" join (the prior implementation) silently
...
// perfectly healthy server ("did not become ready"), and every retry launches
// a redundant herdr server contending for the same pane ("agent_pane_busy") —
// ga-nqlb8q. The same hardcoded "~/.config" made the provider inoperable on
// Windows, where herdr writes to %AppData%\herdr\sessions\<session>\herdr.sock.
//
