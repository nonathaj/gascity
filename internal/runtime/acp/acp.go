...
func (p *Provider) GetMeta(name, key string) (string, error) {
	// Retrying read: publishActivity replaces this sidecar with WriteFileAtomic, and on
	// Windows a reader that opens during the replace gets a sharing violation instead of
	// either version's contents (POSIX renames are atomic to readers, so this window does
	// not exist there).
	data, err := fsys.ReadFileWithTransientRetry(fsys.OSFS{}, p.metaPath(name, key))
	if err != nil {
