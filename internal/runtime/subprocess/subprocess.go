...
func validatePrivateSocketDir(path string, euid int) error {
	...
	// Ownership/permission enforcement is platform-specific: Unix checks the
	// 0700 mode, absence of setuid/setgid/sticky bits, and owner uid; Windows
	// has no Unix mode bits or uid (access is governed by ACLs on a per-user
	// temp path), so it validates what it can. See the _unix/_windows split.
	return validatePrivateSocketOwnership(path, info, euid)
}
