...
//
// Windows cannot byte-range-lock a directory handle, so there the same
// contract is met by a named kernel mutex derived from the directory path:
// cross-process, no entry in the censused namespace, and released by the
// kernel when the holder dies. See openSequenceFloorLock's platform files.
func persistSQLiteSequenceFloorAtLeast(floorPath string, requested int64) (persisted int64, returnErr error) {
	lock, err := openSequenceFloorLock(filepath.Dir(floorPath))
	if err != nil {
