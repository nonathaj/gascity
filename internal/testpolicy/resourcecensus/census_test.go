...
func TestBootstrapPolicyOwnsNetListenDebtAndExactMediumOwners(t *testing.T) {
	...
	if debt.BaselineCalls != 95 || debt.BaselineFiles != 35 || debt.ReportedCalls != 92 || debt.ReportedFiles != 34 {
		t.Fatalf("stream-listener source baseline/reported = %d/%d, %d/%d; want 95/35, 92/34", debt.BaselineCalls, debt.BaselineFiles, debt.ReportedCalls, debt.ReportedFiles)
	}
	smallDebt := findRow(t, bootstrapPolicy.SmallDebt, ScopeUntagged, ResourceNetListen)
	if smallDebt.BaselineCalls != 93 || smallDebt.BaselineFiles != 34 {
		t.Fatalf("stream-listener Small baseline = %d/%d, want 93/34", smallDebt.BaselineCalls, smallDebt.BaselineFiles)
	}
