...
| SESSION-RECON-002 | Cold pool scale from zero | A min=0 pool with custom scale check returning 0 wakes one session when executable routed work exists in any active store, including city-store work targeting a rig-qualified agent. Demand is clamped to 1. Graph.v2 workflow roots, scope latches, and specs are not executable demand; legacy workflow roots and control steps remain eligible. | `cmd/gc/scale_from_zero_test.go`; `cmd/gc/demand_topology_test.go` |
...
| SESSION-RECON-015 | Workflow topology is not worker demand | Native demand and generated routed queries exclude graph.v2 roots and scope/spec latches. Queries exclude topology before limiting candidates, so more than 20 old latches cannot hide executable tasks. A topology-only queue contributes zero demand; legacy workflow roots and executable control steps remain eligible. | `cmd/gc/demand_topology_test.go`; `internal/config/workquery_topology_test.go`; `internal/beadmeta/workflow_latch.go` |
| SESSION-RECON-016 | City-wide launch admission | Workspace max_active_sessions covers pooled, named and manual launches across controller, CLI and API, including concurrent in-flight starts. Exact qualified session_limit_exempt_templates may exclude non-model infrastructure. Unknown live sessions count; partial observations defer starts. Capacity refusal preserves existing work and never accrues wake failures. Nil or negative limits preserve unlimited launch admission. | `internal/session/start_admission_test.go`; `cmd/gc/session_admission_test.go`; `internal/api/session_admission_test.go` |
