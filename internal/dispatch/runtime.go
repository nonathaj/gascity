...
func finalizeScopeAbort(store beads.Store, rootID, scopeRef string, body, subject, controlBead beads.Bead, opts ProcessOptions) (ControlResult, error) {
	...
	// Propagate non-gc.* member metadata (e.g., review.verdict, preflight.report)
	// onto the scope body before closing, so diagnostics survive the failure
	// auto-close. A hard member failure force-closes the body as failed even when
	// it had already closed as pass: a scope abort always wins over a stale pass
	// outcome. closeScopeBodyAbort records the structured scope-abort failure
	// metadata so the body never strands the run with an empty failure reason.
	if err := tracePhaseErr(opts, controlBead.ID, "propagate-metadata", func() error {
		return snapshot.propagateScopeMemberMetadata(store, body.ID)
	}); err != nil {
		return ControlResult{}, fmt.Errorf("%s: propagating scope metadata: %w", controlBead.ID, err)
...
// as failed. It skips the remaining open members, propagates the member's
// non-gc.* diagnostics onto the body, and force-closes it with structured
// scope-abort failure metadata. The body is force-closed as failed even when it
// had already closed as pass, so a hard member failure always overrides a stale
// pass outcome. It mirrors the scope-fail tail of processScopeCheck for the
// no-control-bead reconcile path.
