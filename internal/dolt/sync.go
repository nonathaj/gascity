...
// sync.go — copied from gastown internal/doltserver/sync.go at 8e7ce630.
// Adapted: package doltserver → dolt, direct function calls (same package),
...
func CommitWorkingSet(dbDir string) error {
	...
	commitCmd := exec.Command("dolt", "commit", "-m", "gc dolt sync: auto-commit working changes")
	commitCmd.Dir = dbDir
