...
func DeletePolecatBranch(townRoot, rigDB, branchName string) {
	if err := validateBranchName(branchName); err != nil {
		printWarning("invalid Dolt branch name %q: %v", branchName, err)
		return
	}
	query := fmt.Sprintf("CALL DOLT_BRANCH('-d', '%s')", branchName)
	if err := doltSQL(townRoot, rigDB, query); err != nil {
		// Non-fatal: branch may not exist (already merged/deleted)
		printWarning("could not delete Dolt branch %s: %v", branchName, err)
	}
}

// ---- Inlined helpers replacing gastown imports (util, style, beads) ----
...
// then atomically renames it to path. Replaces util.AtomicWriteFile from gastown.
...
// Replaces style.PrintWarning from gastown.
