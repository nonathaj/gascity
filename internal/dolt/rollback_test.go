...
func TestFindBackups_IgnoresNonBackupDirs(t *testing.T) {
	...
	// And one real backup
	if err := os.MkdirAll(filepath.Join(townRoot, "migration-backup-20260101-000000"), 0o755); err != nil {
		t.Fatal(err)
	}
...
func TestRestoreFromBackup_FormulaStyle(t *testing.T) {
	...
	if err := os.MkdirAll(townBackupBeads, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(townBackupBeads, "metadata.json"),
		[]byte(`{"database": "beads.db", "backend": "sqlite"}`), 0o644); err != nil {
		t.Fatal(err)
	}

	// Rig-level backup
	rigBackupBeads := filepath.Join(backupDir, "gastown-beads")
	if err := os.MkdirAll(rigBackupBeads, 0o755); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.WriteFile(filepath.Join(townBeads, "metadata.json"),
		[]byte(`{"backend": "dolt", "dolt_mode": "server"}`), 0o644); err != nil {
		t.Fatal(err)
	}
