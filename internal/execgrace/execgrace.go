...
// RecordForceKilled marks the cancellation as delivered by a direct forced
// kill. It is for a platform cancel adapter that cannot interrupt a process
// group — Windows has no cross-process SIGINT — and terminates the tree
// instead. InterruptThenKill records its own outcomes and never needs it.
func (r *CancelResult) RecordForceKilled() { r.record(CancelForceKilled) }
