	...
	t.Run("RenameRejectsFileDirectoryCollisions", func(t *testing.T) {
		...
		t.Run("directory onto file", func(t *testing.T) {
			if runtime.GOOS == "windows" {
				// Windows os.Rename does not reject renaming a directory
				// onto an existing file the way POSIX rename(2) does, so
				// this cross-OS collision contract does not hold there.
				t.Skip("os.Rename directory-onto-file collision semantics differ on Windows")
			}
			filesystem, root := newNamespace(t, newFS)
			directory := filepath.Join(root, "directory")
