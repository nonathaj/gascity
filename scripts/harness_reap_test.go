...
// in its own process group and signaled through it (kill(-pgid)), liveness
...
func (f *reapFixture) start(t *testing.T, extraEnv ...string) *exec.Cmd {
	...
	// processgroup, not a raw SysProcAttr: Setpgid does not exist on Windows,
	// where a new process group is created with a spawn flag instead.
