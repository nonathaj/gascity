#!/usr/bin/env bash
# act3-lifecycle-wasteland.sh — Act 3: Wasteland Auto-Claim (Deterministic Agents)
#
# Demonstrates: External work intake with zero human intervention.
# Uses the real wl binary and a real Wasteland instance, but agents
# are deterministic bash scripts (lifecycle topology with mock-polecat.sh).
#
# Chain: wl -> wasteland-poll.sh -> creates beads -> controller
# -> spawns deterministic polecats -> mock-polecat.sh processes them
#
# Prerequisite: Real `wl` binary installed and a Wasteland instance running.
#
# Usage:
#   ./act3-lifecycle-wasteland.sh
#
# Env vars:
...
DEMO_CITY="${DEMO_CITY:-$HOME/lifecycle-demo}"
...
narrate "Act 3: Wasteland Auto-Claim" --sub "External work intake — deterministic agents"
pause
...
# ── Initialize city with lifecycle + wasteland-feeder ─────────────────────

step "Initializing city with lifecycle + wasteland-feeder topologies..."
...
step "City configured with lifecycle + wasteland-feeder"
...
step "Rig registered: demo-repo -> topologies/lifecycle"
...
echo "  5. Deterministic polecat spawns, processes bead, exits"
...
narrate "Act 3 Complete" --sub "Wasteland items claimed, dispatched, and completed (deterministic agents)"
