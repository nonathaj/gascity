#!/usr/bin/env bash
# act1-topology-escalation.sh — Act 1: Progressive Topology Escalation
#
...
#
# The script sets up all infrastructure (topologies, rig beads, demo repo)
...
# We initialize everything (topologies, rig, beads) so the presenter only
# needs to edit city.toml — no commands to run during the demo.

step "Setting up infrastructure (topologies, rig, beads)..."
...
# Copy additional topologies locally (remote fetch doesn't resolve formula
# layers yet, so automations need local paths to be discovered).
cp -r "$GC_SRC/examples/swarm-lifecycle/topologies/swarm-lifecycle" "$DEMO_CITY/topologies/"
cp -r "$GC_SRC/examples/wasteland-feeder" "$DEMO_CITY/topologies/"

# Clone demo repo (lifecycle topology needs push/pull).
DEMO_REPO="$DEMO_CITY/demo-repo"
git clone -q https://github.com/julianknutsen/gc-demo-repo "$DEMO_REPO"

# Register the rig (sets up routes, hooks, topology config).
...
step "Rig infrastructure ready (routes, hooks, topology)"

# ── Bootstrap city.toml for init phase ─────────────────────────────────
# Write a minimal config with the rig but NO topology includes. This lets
...
# Separate beads for each topology so the demo shows distinct work per step.
...
# Override remote topology automation interval for demo speed.
...
# ── Uncomment after Step 3 (lifecycle topology defines the polecat agent) ──
# [[patches.agents]]
# dir = ""
# name = "polecat"
# [patches.agents.env]
