...
| ~~Order system~~ | **RESOLVED:** Orders are formulas with gate frontmatter. Deacon reads order dir, checks gate conditions (filesystem + state.json), executes if open. No gt/gc/bd order commands — all prompt-level. Spec §16 is Tutorial 05c territory. | deacon-patrol |
