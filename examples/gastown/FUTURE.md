...
| ~~Automation system~~ | **RESOLVED:** Automations are formulas with gate frontmatter. Deacon reads automation dir, checks gate conditions (filesystem + state.json), executes if open. No gt/gc/bd automation commands — all prompt-level. Spec §16 is Tutorial 05c territory. | deacon-patrol |
