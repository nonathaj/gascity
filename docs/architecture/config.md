...
Describes how config loading, composition, topology expansion, provider
...
configuration from `city.toml`, included fragments, topology directories,
and remote topology sources into a single flat `City` struct that drives
...
resolution ensures that topology defaults can be customized per-rig
without forking.
...
- **Progressive Activation**: Capabilities emerge from config section
  ...
  activates health monitoring. Adding `[[rigs]]` with topologies
...
- **Topology**: A reusable agent configuration directory containing
  `topology.toml`, prompts, formulas, and automations. City-level
  topologies stamp city-scoped agents (dir=""). Rig-level topologies
  stamp rig-scoped agents (dir=rig-name). The `city_agents` metadata
  field partitions which agents from a shared topology are city-scoped
  vs rig-scoped.
...
- **Override Resolution**: A four-layer chain that allows progressively
  ...
  `[providers]` < workspace defaults < per-agent fields. For topology
  agents, rig-level `[[overrides]]` and city-level `[patches]` provide
  additional override points without forking the topology.
...
- **Revision**: A deterministic SHA-256 hash computed from all config
  source file contents plus topology directory contents. The controller
...
2. Load & merge fragments    (mergeFragment for each include)
    |
    v
3. Resolve named topologies  (resolveNamedTopologies: name -> cache path)
    |
    v
4. Expand city topologies    (ExpandCityTopologies: stamp dir="" agents)
    |
    v
5. Apply patches             (ApplyPatches: targeted field modifications)
    |
    v
6. Expand rig topologies     (ExpandTopologies: stamp dir=rig-name agents)
    |
    v
...
Steps 4 and 6 are ordered deliberately: city topologies expand before
patches so that patches can target city-topology agents. Rig topologies
...
- **`City`** (`internal/config/config.go`): Top-level config struct.
  Contains Workspace, Agents, Rigs, Providers, Topologies, Patches,
...
- **`AgentOverride`** (`internal/config/config.go`): Modifies a
  topology-stamped agent for a specific rig. Same pointer-field
  semantics as AgentPatch. Applied during `ExpandTopologies`.
...
- **`TopologySource`** (`internal/config/config.go`): Defines a remote
  topology repository with git URL, ref, and optional subdirectory path.
  Referenced by name in workspace/rig topology fields.

- **`TopologyMeta`** (`internal/config/config.go`): Metadata header from
  `topology.toml`. Contains name, version, schema version, optional
...
- **`FormulaLayers`** (`internal/config/config.go`): Holds resolved
  ...
  Priority order (lowest to highest): city-topology < city-local <
  rig-topology < rig-local.
...
- **Agent identity uniqueness.** No two agents in the resolved config
  ...
  When duplicates arise from topology expansion, provenance (SourceDir)
...
- **Topology schema compatibility.** `loadTopology` rejects any
  topology with `schema` > `currentTopologySchema` (currently 1).
  Forward-incompatible topologies fail loudly.

- **city_agents names must exist.** Every name listed in a topology's
  `city_agents` must match an agent defined in that topology.
  `loadTopology` validates this before any agent stamping.
...
- **Revision determinism.** Given identical file contents, `Revision`
  ...
  hashing, and topology content is hashed recursively with sorted
  relative paths.
...
| Depends on | How |
|---|---|
| `internal/fsys` | Filesystem abstraction for `Load`, `LoadWithIncludes`, topology loading, and revision hashing |
...
| File | Purpose |
|---|---|
| `internal/config/config.go` | Core types: `City`, `Workspace`, `Agent`, `Rig`, `AgentOverride`, `TopologySource`, `TopologyMeta`, `FormulaLayers`, `PoolConfig`, subsystem configs. Load/Parse/Marshal. Validation functions. |
...
| `internal/config/topology.go` | `ExpandTopologies`, `ExpandCityTopologies`, `ComputeFormulaLayers`. Topology loading, agent stamping, city_agents partitioning, override application, collision detection. |
| `internal/config/topology_fetch.go` | `FetchTopologies`: git clone/update for remote topology sources. `TopologyLock` for reproducible builds. Cache management under `.gc/topologies/`. |
...
Multi-rig with topology and overrides (Level 5+):

```toml
[workspace]
name = "my-city"
provider = "claude"
topology = "topologies/my-topology"

[topologies.shared]
source = "https://github.com/example/topologies.git"
ref = "v1.0"
path = "my-topology"
...
1. City topology formulas (from `workspace.topology` or `workspace.topologies`)
2. City local formulas (from `[formulas] dir`)
3. Rig topology formulas (from `rigs[].topology` or `rigs[].topologies`)
...
| `internal/config/topology_test.go` | ExpandTopologies, ExpandCityTopologies, city_agents partitioning, agent collision detection, override application, formula layer computation |
| `internal/config/topology_fetch_test.go` | FetchTopologies, clone/update, TopologyCachePath, lock read/write, LockFromCache |
...
- **Remote topology fetch requires git.** `FetchTopologies` shells out
  to `git clone`/`git fetch`. There is no fallback for environments
  without git.

- **Shallow clones only.** Remote topologies are cloned with
  `--depth 1`. Switching to a commit ref that is not at the tip of a
  branch may fail.

- **No hot-reload of topology content.** The controller watches config
  source files and reloads on change, but topology directories are only
  re-hashed during revision computation. Changes to files within a
  topology directory are detected, but new files added outside the
...
- [Glossary](./glossary.md) -- authoritative definitions of all Gas City
  terms, including Config, Topology, Rig, and Provider
