...
// TestStatusTextKeepsGutterOnLongNamedSessionAndRigRows extends the
// padStatusName gutter guarantee (gastownhall/gascity#4579) past the Agents
// block: the "Named sessions:" and "Rigs:" rows were still rendered with a
// bare "%-24s", so an identity or rig name at or past the column width ran
// straight into the token after it ("gem-federation-long-rig-name/home/…").
func TestStatusTextKeepsGutterOnLongNamedSessionAndRigRows(t *testing.T) {
	const (
		longIdentity = "gem-federation/gc.run-operator-1" // 32 runes, past the 24-wide column
		longRig      = "gem-federation-long-rig-name"     // 28 runes
	)
	snapshot := cityStatusSnapshot{
		CityName: "testcity",
		CityPath: "/tmp/testcity",
		NamedSessions: []cityStatusNamedSession{
			{Identity: longIdentity, Status: "running", Mode: "pinned"},
		},
		Rigs: []StatusRigJSON{
			{Name: longRig, Path: "/tmp/testcity/rigs/" + longRig},
		},
	}
	var stdout bytes.Buffer
	renderCityStatusText(snapshot, newFakeDrainOps(), &stdout)
	...
	for _, tc := range []struct{ name, joined, gutter string }{
		{"named session", longIdentity + "running", longIdentity + strings.Repeat(" ", statusNameColumnGutter) + "running"},
		{"rig", longRig + "/tmp", longRig + strings.Repeat(" ", statusNameColumnGutter) + "/tmp"},
	} {
		if strings.Contains(out, tc.joined) {
			t.Fatalf("%s row lost its gutter: %q ran into the next token\n%s", tc.name, tc.joined, out)
		}
		if !strings.Contains(out, tc.gutter) {
			t.Fatalf("%s row missing %d-space gutter after %q\n%s", tc.name, statusNameColumnGutter, tc.name, out)
		}
	}
}
