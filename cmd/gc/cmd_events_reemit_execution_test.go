...
func TestEventsReemitExecutionHoldsControllerLockUntilCompletion(t *testing.T) {
	...
	// fslock, not syscall.Flock: this probes the very lock the command under
	// test holds, which requireStoppedExecutionReemitCity takes through fslock
	// (flock(2) on Unix, a byte-0 LockFileEx range on Windows). Any other
	// primitive would either not compile on Windows or not contend with the
	// holder.
	if err := fslock.TryLockEx(competitor); !fslock.WouldBlock(err) {
		t.Fatalf("competing controller lock = %v, want a would-block error", err)
	}
