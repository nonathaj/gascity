...
func runEventsReemitExecution(cmd *cobra.Command, runID string, apply bool, stdout io.Writer) error {
	...
	defer func() {
		_ = fslock.Unlock(controllerLock)
...
func requireStoppedExecutionReemitCity(cityPath string) (*os.File, error) {
	...
	// fslock, not syscall.Flock: this is the same controller.lock the running
	// controller holds, and on Windows that lock is a LockFileEx byte range.
	// Probing it with flock would not compile there, and probing it with
	// anything else would not contend with the holder.
