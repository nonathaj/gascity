...
// loadCityConfigPrematerializedCalls counts loads that take the builtin packs
// as they already are on disk (loadPrematerializedCityConfig). Those are still
// full config resolutions — they parse city.toml and every include — they just
// decline the pack refresh, so loadCityConfigCalls above cannot see them.
//
// The store-open path deliberately uses that cheaper loader, which is why a
// test asking "did a config load happen at all?" must consult
// cityConfigLoadCount rather than loadCityConfigCalls alone.
var loadCityConfigPrematerializedCalls atomic.Int64

// cityConfigLoadCount is every full config resolution by either loader.
func cityConfigLoadCount() int64 {
	return loadCityConfigCalls.Load() + loadCityConfigPrematerializedCalls.Load()
}
...
func loadPrematerializedCityConfig(fs fsys.FS, tomlPath string, opts config.LoadOptions, warnings io.Writer) (*config.City, error) {
	loadCityConfigPrematerializedCalls.Add(1)
