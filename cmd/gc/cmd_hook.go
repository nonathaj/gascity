...
// "180s").
//
// The default is platform-aware. 2026-08-14: the Unix default was raised
// 60s -> 150s. Measured on a loaded six-rig city: each `gc ready` leg of the
// default probe costs 10-14s (~4s process start + a 6-leg federated read), and
// the five sequential reads put the pool-demand payoff call at t=60s exactly —
// 850+ session.work_query_failed events with every hook starved while
// `gc ready` run standalone returned the routed rows. 150s covers the measured
// worst case (~70s) with margin for load; the real cure remains ga-4qdfn
// (fewer round-trips, faster reader).
//
// Windows is higher still because the composite probe is far more expensive
// there: each bd invocation pays per-call doltlite fallback at roughly 4s per
// round-trip, and every sh construct in the probe is a process spawn costing
// ~380ms where Unix forks in under a millisecond. Six sequential round-trips
// plus the probe's own shell work exceed 60s routinely. That cost was
// previously documented here but left to an env var each operator had to
// discover and set; without it the serve loop emitted
