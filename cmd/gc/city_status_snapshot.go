...
func renderCityStatusText(snapshot cityStatusSnapshot, dops drainOps, stdout io.Writer) {
	...
	if len(snapshot.NamedSessions) > 0 {
		...
		for _, named := range snapshot.NamedSessions {
			fmt.Fprintf(stdout, "  %s%s (%s)\n", padStatusName(named.Identity, statusNameColumnWidth), named.Status, named.Mode) //nolint:errcheck // best-effort stdout
		}
	}
	...
	if len(snapshot.Rigs) > 0 {
		...
		for _, r := range snapshot.Rigs {
			...
			fmt.Fprintf(stdout, "  %s%s%s\n", padStatusName(r.Name, statusNameColumnWidth), r.Path, annotation) //nolint:errcheck // best-effort stdout
		}
	}
...
func renderConditionalWritesBlock(stdout io.Writer, cw *api.StatusConditionalWrites) {
	...
	for _, v := range cw.Stores {
		if v.Capable {
			fmt.Fprintf(stdout, "  %s%-8scapable\n", padStatusName(v.StoreID, statusNameColumnWidth), v.Kind) //nolint:errcheck // best-effort stdout
			continue
		}
		fmt.Fprintf(stdout, "  %s%-8sINCAPABLE (probe=%s latch=%s): %s\n", padStatusName(v.StoreID, statusNameColumnWidth), v.Kind, v.Probe, v.Latch, v.Reason) //nolint:errcheck // best-effort stdout
	}
