...
and its root bead is routed to the target.`,
		...
		RunE: func(_ *cobra.Command, args []string) error {
			code := cmdSling(args, formula, nudge, force, title, onFormula, noFormula, dryRun, stdout, stderr)
			if code != 0 {
				return errExit
			}
			return nil
...
func cmdSling(args []string, isFormula, doNudge, force bool, title, onFormula string, noFormula, dryRun bool, stdout, stderr io.Writer) int {
...
func doSling(opts slingOpts, deps slingDeps, querier BeadQuerier) int {
	...
	if opts.IsFormula {
		method = "formula"
		rootID, err := instantiateWisp(opts.BeadOrFormula, opts.Title, deps.Runner)
		if err != nil {
	...
	if opts.OnFormula != "" {
		...
		wispRootID, err := instantiateWispOn(opts.OnFormula, beadID, opts.Title, deps.Runner)
		if err != nil {
	...
	if opts.OnFormula == "" && !opts.IsFormula && !opts.NoFormula && a.DefaultSlingFormula != "" {
		...
		wispRootID, err := instantiateWispOn(a.DefaultSlingFormula, beadID, opts.Title, deps.Runner)
		if err != nil {
...
func doSlingBatch(opts slingOpts, deps slingDeps, querier BeadChildQuerier) int {
	...
	for _, child := range open {
		...
		// Attach wisp if --on.
		if opts.OnFormula != "" {
			wispRootID, err := instantiateWispOn(opts.OnFormula, child.ID, opts.Title, deps.Runner)
			if err != nil {
			...
			wispRootID, err := instantiateWispOn(a.DefaultSlingFormula, child.ID, opts.Title, deps.Runner)
			if err != nil {
...
func instantiateWisp(formulaName, title string, runner SlingRunner) (string, error) {
	cmd := "bd mol cook --formula=" + shellQuote(formulaName)
	if title != "" {
...
func instantiateWispOn(formulaName, beadID, title string, runner SlingRunner) (string, error) {
	cmd := "bd mol cook --formula=" + shellQuote(formulaName) + " --on=" + shellQuote(beadID)
	if title != "" {
		cmd += " --title=" + shellQuote(title)
	}
	out, err := runner(cmd)
...
func dryRunSingle(opts slingOpts, deps slingDeps, querier BeadQuerier) int {
	...
	if opts.IsFormula {
		...
		w("")
		cookCmd := "bd mol cook --formula=" + opts.BeadOrFormula
		if opts.Title != "" {
			cookCmd += " --title=" + opts.Title
		}
		w("  Would run: " + cookCmd)
		...
		if opts.OnFormula != "" {
			...
			w("")
			cookCmd := "bd mol cook --formula=" + opts.OnFormula + " --on=" + opts.BeadOrFormula
			if opts.Title != "" {
				cookCmd += " --title=" + opts.Title
			}
			w("  Would run: " + cookCmd)
			...
			w("")
			cookCmd := "bd mol cook --formula=" + a.DefaultSlingFormula + " --on=" + opts.BeadOrFormula
			if opts.Title != "" {
				cookCmd += " --title=" + opts.Title
			}
			w("  Would run: " + cookCmd)
